...
let handleMessage = async (sender_psid, message) => {
    ...
    if (message && message.quick_reply && message.quick_reply.payload) {
        if (message.quick_reply.payload === "CHEAP" || message.quick_reply.payload === "FAIR" || message.quick_reply.payload === "EXPENSIVE") {
            //asking about phone number
            if (message.quick_reply.payload === "CHEAP") user.quantity = "KSH 1000";
            if (message.quick_reply.payload === "FAIR") user.quantity = "KSH 2000";
            if (message.quick_reply.payload === "EXPENSIVE") user.quantity = "KSH 3000";
    ...
    if (entity.name === "wit$datetime:datetime") {
        ...
        await chatBotService.sendResponseGreetings(sender_psid, locale);
...
let handlePostback = async (sender_psid, received_postback) => {
    ...
    switch (payload) {
        ...
        case "GUIDE_BOT":
            ...
            break; 
		case "GUIDE_BOT":
            await chatBotService.sendTalkToUs(sender_psid);
            break;
        case "MEN_WEAR":
            await chatBotService.sendMenWear(sender_psid);
            break;
        case "WOMEN_WEAR":
            await chatBotService.sendWomenWear(sender_psid);
            break;
        case "TEENAGE_TSHIRTS":
            await chatBotService.sendTeengeTshirts(sender_psid);
            break;
        case "TEENAGE_HOODIES":
            await chatBotService.sendTeengeHoodies(sender_psid);
            break;
        case "TEENAGE_WEAR":
            await chatBotService.sendTeenageWear(sender_psid);
            break;
        case "WOMEN_SUITS":
            await chatBotService.sendWomenSuits(sender_psid);
            break;
        case "MEN_TSHIRTS":
            await chatBotService.sendMenTshirts(sender_psid);
            break;

        case "MEN_HOODIES":
            await chatBotService.sendMenHoodies(sender_psid);
            break;
        case "SHADES_CAPS":
            await chatBotService.sendShadesCaps(sender_psid);
            break;
        case "WOMEN_HOODIES":
            await chatBotService.sendWomenHoodies(sender_psid);
            break;

        case "BACK_TO_MAIN_MENU":
            await chatBotService.goBackToMainMenu(sender_psid);
            break;
        case "BACK_TO_MEN_WEAR":
            await chatBotService.goBackTosendMenWear(sender_psid);
            break;
